...
def main():
    ...
    if os.name=="nt":
        path_to_openai_server_venv_python = PATH_TO_OPENAI_MCP_SERVER_PROJECT / ".venv" / "Scripts"/ "python.exe"
    else:
        path_to_openai_server_venv_python = PATH_TO_OPENAI_MCP_SERVER_PROJECT / ".venv" / "bin" / "python"
    ...
    mcp_config = {
        "initial_servers": {
            "my_openai_server": {
                "command": [
                    sys.executable, # Explicitly specify the Python interpreter
                    "openai-mcp-server/openai_mcp_server/server.py" # Full path to your server script
                ],
